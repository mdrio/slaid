...
class FilteredRunner(Runner):
    ...
    def _convert_condition(self) -> Filter:
        ...
        parsed = re.match(
            r"(?P<mask>\w+)\s*(?P<operator>[<>=!]+)\s*(?P<value>\d+\.*\d*)", condition
        ).groupdict()
        label = parsed["mask"]
    ...
    def classifier(self):
        if self._classifier is None:
            self._classifier = BasicClassifier(
                self.model,
                self.label,
                chunk=(self.chunk_size, self.chunk_size) if self.chunk_size else None,
...
class FilteredPatchRunner(FilteredRunner):
    def __post_init__(self):
        ...
        if self.model.patch_size is None:
            raise RuntimeError(f"model {self.model_name} does not work with patch")
    ...
    def classifier(self):
        if self._classifier is None:
            self._classifier = FilteredPatchClassifier(
                self.model, self.label, self._filter_obj
            )
        return self._classifier
...
class PixelRunner(Runner):
    ...
    def classifier(self):
        if self._classifier is None:
            self._classifier = PixelClassifier(
                self.model, self.label, chunk_size=self.chunk_size
            )
