...
class Classifier(abc.ABC):
    ...
    def __init__(self, model: "Model", label: str, array_factory: ArrayFactory = None):
        self.model = model
        self.label = label
    ...
    def _get_mask(self, slide, array, level, downsample, round_to_0_100, tile_size=1):
...
class BasicClassifier(Classifier):
    ...
    def classify(
        ...
        if self._patch_size:
            predictions = self._classify_patches(
                slide_array, chunk, threshold, round_to_0_100, dtype
            )
        else:
            predictions = self._classify_pixels(
                slide_array, chunk, threshold, round_to_0_100, dtype
            )
        return self._get_mask(
            slide, predictions, level, slide.level_downsamples[level], round_to_0_100
        )

    def _classify_pixels(self, slide_array, chunk, threshold, round_to_0_100, dtype):
        if self._filter:
        ...
        predictions = self.array_factory.empty((0, slide_array.size[1]), dtype=dtype)
        with Bar("Processing", max=_filter.shape[0] // chunk[0] or 1) as bar:
            for x in range(0, _filter.shape[0], chunk[0]):
                row = np.empty((min(chunk[0], _filter.shape[0] - x), 0), dtype=dtype)
                for y in range(0, _filter.shape[1], chunk[1]):
                    ...
                    if (filter_block == True).any():
                        block = slide_array[x : x + chunk[0], y : y + chunk[1]].convert(
    ...
    def _classify_patches(self, slide_array, chunk, threshold, round_to_0_100, dtype):
        ...
        for i in range(2):
            if chunk[i] % self._patch_size[i] > 0 and chunk[i] < slide_array.size[i]:
                raise InvalidChunkSize(
                    f"Invalid chunk size {chunk}: should be a multiple of {self._patch_size}"
                )
        with Bar(
            "Processing", max=slide_array.size[0] // chunk[0] or 1
        ) as progress_bar:
            for x in range(0, slide_array.size[0], chunk[0]):
                ...
                if not row_size:
                    break
                row = np.empty((row_size // self._patch_size[0], 0), dtype=dtype)
                ...
                for y in range(0, slide_array.size[1], chunk[1]):
                    ...
                    filter_block = _filter[
                        x
                        // self._patch_size[0] : (x + row_size)
                        // self._patch_size[0],
                        y
                        // self._patch_size[1] : (y + col_size)
                        // self._patch_size[1],
                    ]
                    res = np.zeros(filter_block.shape, dtype="float32")

                    if (filter_block == True).any():
                        chunked_array = slide_array[
                            x : x + row_size, y : y + col_size
                        ].convert(self.model.image_info)
                        patches, channel_first = chunked_array.get_blocks(
                            self._patch_size
                        )
                        to_predict = (
                            patches[:, filter_block, ...]
                            if channel_first
                            else patches[filter_block, :, ...]
                        )
