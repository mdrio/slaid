...
class BatchIterator:
    ...
    def __post_init__(self):
        self._buffer: np.ndarray = np.empty((3, 0) if self.channel_first else (0, 3))
    ...
    def iter(self) -> np.ndarray:
        ...
        for i in range(0, max_index, self.batch_size):
            batch = (
                self._buffer[:, i : i + self.batch_size]
                if self.channel_first
                else self._buffer[i : i + self.batch_size, :]
            )
            yield batch
        self._buffer = (
            self._buffer[:, max_index:]
            if self.channel_first
            else self._buffer[max_index:, :]
        )
    ...
    def _buffer_size(self):
        return self._buffer.shape[1] if self.channel_first else self._buffer.shape[0]


class Classifier(BaseClassifier):
    def _predict_by_batch(
        self, batch_iterator: BatchIterator, all_buffer: bool
    ) -> np.ndarray:
        ...
        predictions = np.concatenate(predictions) if predictions else np.empty((0,))
        return predictions
...
class PixelClassifier(Classifier):
    ...
    def classify(
        ...
        for row_idx in range(0, slide_array.size[0], row_size):
            row = (
                slide_array[row_idx : row_idx + row_size, :]
                .convert(self.model.image_info)
                .array
            )
        ...
        return self._get_mask(
            slide, res, level, slide.level_downsamples[level], round_to_0_100
        )

    def _set_rows(
        self, array, row_splitter: "RowSplitter", threshold: float, round_to_0_100: bool
    ):
        try:
...
class FilteredPatchClassifier(FilteredClassifier):
    def classify(
        ...
        patch_coords = list(
            filter(partial(self._remove_borders, slide_array), iter(patch_coords))
        )
        ...
        for patch_coord in patch_coords:
            ...
            patch = (
                slide_array[x : x + self._patch_size[0], y : y + self._patch_size[1]]
                .convert(self.model.image_info)
                .array
            )
    ...
    def _remove_borders(self, slide_array: np.ndarray, coord: np.ndarray) -> bool:
        return coord[0] <= (slide_array.size[0] - self._patch_size[0]) and coord[1] <= (
            slide_array.size[1] - self._patch_size[1]
        )
...
class FilteredPixelClassifier(FilteredClassifier):
    def classify(
        ...
        if self._patch_size:
            raise RuntimeError(f"Not expecting patch size, found {self._patch_size}")
        ...
        for patch_coord in patch_coords:
            ...
            patch = (
                slide_array[x : x + tile_size[0], y : y + tile_size[1]]
                .convert(self.model.image_info)
                .array
            )
        ...
        to_predict = (
            np.concatenate(patches) if patches else np.empty((0, 3), dtype="uint8")
        )
        channel_first = self.model.image_info.channel == ImageInfo.Channel.FIRST
        batch_iterator = BatchIterator(batch_size, channel_first)
