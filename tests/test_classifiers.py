...
@pytest.mark.parametrize('model', [GreenModel()])
@pytest.mark.parametrize('tile_size', [1024])
def test_classify_with_filter(green_slide, classifier, level,
                              max_MB_prediction):
    filter_level = 2
    ...
    mask = classifier.classify(green_slide,
                               level=level,
...
@pytest.mark.parametrize('model', [GreenModel()])
@pytest.mark.parametrize('tile_size', [1024])
def test_classify_with_zeros_as_filter(green_slide, classifier, level,
                                       max_MB_prediction):
    filter_level = 2
    filter_downsample = green_slide.level_downsamples[filter_level]
    filter_array = np.zeros(green_slide.level_dimensions[filter_level][::-1])
